...
def segregationEvolution():
    ...
    for index, row in dfCotes.iterrows():
        ...
        for i in list(range(11, 0, -1)):
            coteFin = dfCotes.loc[
                df.index[index], "odds_pmu_chronological_" + str(i) + "_ratio"
            ]
...
def predsModel(modelName, X, y):
    ...
    if modelName == "linearReg":
        ...
        model = SVR(kernel="rbf", C=50, gamma=0.1, epsilon=0.1)
    model.fit(X, y)
...
epochNames = [
    col for col in filterCol if col.endswith("epoch") or col.endswith("epochMs")
]
ratioNames.remove("odds_pmu_reference10h_ratio")
print(
    "\n Nom des colomnes des ratio:",
    ratioNames,
    "\n \n Nom des colomnes des epochs: ",
    epochNames,
    "\n",
)
